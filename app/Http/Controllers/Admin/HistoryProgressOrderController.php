...
class HistoryProgressOrderController extends Controller
{
    ...
    public function store(Request $request, $batch_id)
    {
        $request->validate([
            ...
            // 🟢 operator hanya wajib kalau barisnya benar-benar diisi (lihat validasi di bawah)
        ...
        try {
            ...
            // 🔹 Validasi + ambil hanya baris yang diisi (total > 0). Baris 0 = bypass,
            //    tidak disimpan ke database sama sekali.
            $filledItems = [];
            ...
            foreach ($request->items as $data) {
                ...
                // 🟡 baris kosong → dilewati, operator tidak wajib
                if ($totalInput <= 0) {
                    continue;
                }
                ...
                // ⛔ operator wajib kalau completed/reject/defect diisi
                if (empty($data['operator_id'])) {
                    DB::rollBack();
                    return back()->with('error', "Operator wajib dipilih untuk produk {$assign->progressItem->product->name}.");
                }
            ...
            if (empty($filledItems)) {
                DB::rollBack();
                return back()->with('error', 'Tidak ada progress yang diisi.');
            }
    ...
    public function storeByMachine(Request $request, $machine)
    {
        ...
        try {
            ...
            // 🔹 Validasi sebelum menyimpan apa pun. Baris dengan total 0 (bypass)
            //    dilewati — tidak masuk database dan operatornya tidak wajib.
            $filledItems = [];

    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card stretch stretch-full">
                <form action="/erp/products/update/{{ $product->id }}" method="POST" id="productForm" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="row mb-3 align-items-center">
                            ...
                            <div class="col-lg-10 mb-0">
                                ...
                                <div class="mt-2" id="old-image-container">
                                    <img src="{{ $imageUrl }}" alt="Old Image" id="old-image" style="max-width: 100px; border-radius: 10px">
                                </div>
                                @endif

                                <div class="mt-2" id="new-image-container" style="display: none;">
                                    <img id="preview-image" src="#" alt="Preview" style="max-width: 100px; border-radius: 10px">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row mb-3 align-items-center">
                            ...
                            <div class="col-lg-10 mb-0">
                                <div class="input-group">
                                    <input type="text" class="form-control" id="sku" name="sku" value="{{ old('sku', $product->sku) }}" placeholder="SKU">
                                </div>
                            </div>
                        </div>
                        <div class="row mb-3 align-items-center">
                            ...
                            <div class="col-lg-10 mb-0">
                                <div class="input-group">
                                    <select class="form-control" data-select2-selector="status" id="stock" name="stock">
                                        <option value="instock" data-bg="bg-success" {{ $product->stock == 'instock' ? 'selected' : '' }}>In Stock</option>
                                        <option value="outofstock" data-bg="bg-warning" {{ $product->stock == 'outofstock' ? 'selected' : '' }}>Out Of Stock</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="col-lg-10 mb-0">
                                <div class="input-group">
                                    ...
                                    <select class="form-select form-control max-select" data-select2-selector="tag" multiple id="categories" name="categories[]">
                                        ...
                                        $selected = in_array($category->id, old('categories', $product->categories->pluck('id')->toArray() ?? [])) ? 'selected' : '';
                                        @endphp
                                        <option value="{{ $category->id }}" data-bg="{{ $bg }}" {{ $selected }}>
                                            {{ $category->name }}
                                        </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-lg-10 mb-0">
                                <div class="input-group">
                                    ...
                                    <select class="form-select form-control max-select" data-select2-selector="tag" multiple id="tags" name="tags[]">
                                        ...
                                        @php
                                        $bg = $bgColors[$index % count($bgColors)];
                                        $selected = in_array($tag->id, old('tags', $product->tags->pluck('id')->toArray() ?? [])) ? 'selected' : '';
                                        @endphp
                                        <option value="{{ $tag->id }}" data-bg="{{ $bg }}" {{ $selected }}>
                                            {{ $tag->name }}
                                        </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="row mb-3 align-items-center">
                            ...
                            <div class="col-lg-10 mb-0">
                                <div class="input-group">
                                    <textarea name="description" id="description" cols="30" rows="5" placeholder="Description" class="form-control" value="{{ old('description') }}">{{ strip_tags(old('description', $product->description)) }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>

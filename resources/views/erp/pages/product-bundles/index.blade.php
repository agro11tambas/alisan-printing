...
<script>
    $(document).ready(function() {
        const dataTable = $('#productBundleTable').DataTable({
            processing: true,
            serverSide: true,
            deferRender: true, // 🔹 render baris hanya saat diperlukan
            scrollY: 600, // 🔹 tinggi scroll container
            scroller: true, // 🔹 aktifkan infinite scroll
            paging: true, // tetap bisa paging
            searching: false,
            lengthChange: false,
            info: false, // kalau mau sembunyiin info "Showing 1 to ..."
            pagingType: "simple",
            ajax: {
        ...
        $(document).on('click', function(e) {
            if (!$(e.target).closest('#productBundleTable tbody tr, #productBundleTableMobile tbody tr').length) {
                $('#productBundleTable tbody tr.shown, #productBundleTableMobile tbody tr.shown').each(function() {
                    var tr = $(this);
                    var table = tr.closest('table').attr('id') === 'productBundleTable' ? dataTable : dataTableMobile;

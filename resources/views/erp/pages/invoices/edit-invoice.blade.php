    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card stretch stretch-full">
                <form action="/erp/invoices/update/{{ $invoice->id }}" method="POST" id="invoiceForm" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                            ...
                            <div class="col-lg-10 mb-0">
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control" id="account_number" name="account_number"
                                           value="{{ old('account_number', $invoice->account_number) }}" placeholder="Account Number">
                                </div>
                            </div>
                        </div>
                        <div class="row mb-3 align-items-center">
                                ...
                                <div id="contents-wrapper">
                                    @if($invoice->termAndConditions && $invoice->termAndConditions->count())
                                        @foreach ($invoice->termAndConditions as $index => $content)
                                            <div class="content-item mb-2 d-flex align-items-start">
                                                <textarea class="form-control" name="contents[]" rows="1"
                                                          placeholder="Enter content">{{ old("contents.$index", $content->content) }}</textarea>
                                                <button type="button"
                                    ...
                                    @else
                                        <div class="content-item mb-2 d-flex align-items-start">
                                            ...
                                            <button type="button" class="btn btn-danger ms-2 remove-content d-none">Remove</button>
                                        </div>
                                    @endif
                                </div>
                                <button type="button" class="btn btn-success mt-2" id="add-content-btn">+ Add Content</button>
                            </div>
                        </div>
                    </div>
                </form>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        form.addEventListener('submit', function(e) {
            ...
            const rules = [
                { selector: 'input[name="name"]', message: 'Nama wajib diisi' },
                { selector: 'input[name="bank_name"]', message: 'Nama Bank wajib diisi' },
                { selector: 'input[name="account_number"]', message: 'No Rekening wajib diisi' },
            ];

    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card stretch stretch-full">
                <form action="/erp/expenses/update/{{ $expense->transaction_group_id }}" method="POST" id="expenseForm">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="row mb-3 align-items-center">
                            ...
                            <div class="col-lg-10 mb-0">
                                <div class="input-group">
                                    <input type="date" class="form-control" id="transaction_date" name="transaction_date" value="{{ old('transaction_date', $expense->transaction_date ? \Carbon\Carbon::parse($expense->transaction_date)->format('Y-m-d') : '') }}">
                                </div>
                            </div>
                        </div>
                        <div class="row mb-3 align-items-center">
                            ...
                            <div class="col-lg-10 mb-0">
                                <div class="input-group">
                                    ...
                                    <select class="form-select form-control max-select" data-select2-selector="tag" id="transaction_type" name="transaction_type">
                                        ...
                                        @endphp
                                        <option value="{{ $transactionType->id }}" data-bg="{{ $bg }}" {{ $transactionType->id == $expense->account_id ? 'selected' : '' }}>{{ $transactionType->type }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="row mb-3 align-items-center">
                            ...
                            <div class="col-lg-10">
                                <select class="form-control" name="cash_bank_account_id">
                                    ...
                                    <option value="{{ $cash->id }}" {{ $cash->id == $creditTransaction->account_id ? 'selected' : '' }}>
                                        Cash - {{ $cash->type }}
                                    </option>
                                    @endforeach
                                    @foreach ($bankAccounts as $bank)
                                    <option value="{{ $bank->id }}" {{ $bank->id == $creditTransaction->account_id ? 'selected' : '' }}>
                                        Bank - {{ $bank->type }}
                                    </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-3 align-items-center">
                            ...
                            <div class="col-lg-10 mb-0">
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control" id="debit" name="debit" value="{{ old('debit', $expense->debit) }}" placeholder="amount">
                                </div>
                            </div>
                        </div>
                        <div class="row mb-3 align-items-center">
                            ...
                            <div class="col-lg-10 mb-0">
                                <div class="input-group">
                                    ...
                                    <textarea name="note" id="note" cols="30" rows="5" placeholder="note" class="form-control" value="{{ old('note') }}">{{ strip_tags(old('note', $expense->note)) }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>

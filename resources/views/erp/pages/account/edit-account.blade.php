                                ...
                                <div class="col-lg-10 mb-0">
                                    <div class="input-group">
                                        <select class="form-select form-control max-select" data-select2-selector="tag"
                                            ...
                                            <option value="Bank" data-bg="bg-success"
                                                {{ $account->name == 'Bank' ? 'selected' : '' }}>Bank</option>
                                            <option value="Cash" data-bg="bg-primary"
                                                {{ $account->name == 'Cash' ? 'selected' : '' }}>Cash</option>
                                            <option value="Sale" data-bg="bg-dark"
                                                {{ $account->name == 'Sale' ? 'selected' : '' }}>Sale</option>
                                            <option value="Purchase" data-bg="bg-teal"
                                                {{ $account->name == 'Purchase' ? 'selected' : '' }}>Purchase</option>
                                            <option value="Expense" data-bg="bg-warning"
                                                {{ $account->name == 'Expense' ? 'selected' : '' }}>Expense</option>
                                            <option value="Capital" data-bg="bg-danger"
                                                {{ $account->name == 'Capital' ? 'selected' : '' }}>Capital</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
...
@push('scripts')
    <script>
        document.getElementById('accountForm').addEventListener('submit', function(e) {
            ...
            const rules = [{
                    selector: 'select[name="name"]', // ⬅️ ubah karena name adalah select
            ...
            // 🔥 submit pakai request normal agar method PUT terbaca
            if (isValid) {
                e.target.submit(); // ✅ bukan form.submit()
            }
        });

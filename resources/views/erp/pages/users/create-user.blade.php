                                    ...
                                    <div class="col-md-4 mb-3">
                                        <div class="card p-2 h-100">
                                            <div class="form-check mb-2">
                                                ...
                                                <label class="form-check-label fw-semibold" for="perm_{{ $permission->id }}">
                                                    {{ $permission->name }}
                                                </label>
                                            </div>
                                            ...
                                            <div class="ms-3">
                                                ...
                                                <div class="form-check">
                                                    <input class="form-check-input sub-permission"
                                                        type="checkbox"
                                                        name="permission_sub_items[]"
                                                        value="{{ $sub->id }}"
                                                        id="subperm_{{ $sub->id }}"
                                                        data-parent="perm_{{ $permission->id }}"
                                                        {{ in_array($sub->id, old('permission_sub_items', [])) ? 'checked' : '' }}
                                                        {{ in_array($permission->id, old('permissions', [])) ? '' : 'disabled' }}>
                                                    <label class="form-check-label" for="subperm_{{ $sub->id }}">
                                                        {{ $sub->name }}
                                                    </label>
                                                </div>
                                                @endforeach
    ...
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelectorAll('.parent-permission').forEach(function(parentCheckbox) {
            parentCheckbox.addEventListener('change', function() {
                ...
                let subItems = document.querySelectorAll('.sub-permission[data-parent="' + parentId + '"]');

...
@push('styles')
    <style>
        ...
        #ShopManagerList_wrapper .dataTables_scrollBody {
                                ...
                                <div class="row g-3 justify-content-end">
                                    <div class="col-lg-6">
                                        ...
                                        <input type="text" id="name" name="name" class="form-control"
                                            style="padding: 0.5rem 1rem; font-size: 0.875rem;" placeholder="Search Name...">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="table-responsive">
                            <table class="table bg-transparent table-hover" id="ShopManagerList">
                                <thead>
                                    <tr>
    ...
    <script>
        $(document).ready(function() {
            ...
            const dataTable = $('#ShopManagerList').DataTable({
                ...
                pagingType: "simple",
                ajax: {
                    url: "{{ url('/erp/shop-manager/users/data') }}",
                    data: function(d) {
                        d.name = $('#name').val();
                    }
                },
            ...
            $('#ShopManagerList tbody').on('click', 'tr', function(e) {
                if ($(e.target).closest('td.dt-control').length) return;

                let $tr = $(this);
                let row = dataTable.row($tr);

                $('#ShopManagerList tbody tr').removeClass('action-shown').next('.action-row').remove();
            ...
            $(document).on('click', function(e) {
                if ($(e.target).closest('#ShopManagerList').length) return;

                $('#ShopManagerList tbody tr').removeClass('action-shown').next('.action-row').remove();
            });

            $(document).on('click', function(e) {
                if (!$(e.target).closest('#ShopManagerList tbody tr, #ShopManagerListMobile tbody tr')
                    .length) {
                    $('#ShopManagerList tbody tr.shown, #ShopManagerListMobile tbody tr.shown').each(
                        function() {
                            var tr = $(this);
                            var table = tr.closest('table').attr('id') === 'ShopManagerList' ?
